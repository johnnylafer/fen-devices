...
//
//  Same hub sync as v1 (poll 4 s, POST boop). WiFi portal "KURA-BADGE".
// =====================================================================
...
#include "img_kpol1.h"
#include "img_kpol2.h"
#include "img_kura_gif.h"

#define HUB_HOST "fen.188.166.49.216.sslip.io"
#define DEVICE_ID "lcd4-kura"
#define FW_VER "2.0-kura"
...
const uint16_t HOLO[6] = {0x4FEE /*neon green*/, 0xCD09 /*gold*/, 0x4FEE, 0xDF3B /*ink*/, 0xCD09, 0x4FEE};
...
const uint16_t *STK[6] = {kstk1, kstk2, kstk3, kstk4, kstk5, kstk6};
...
void header(const char *title, uint32_t t) {
  ...
  gfx->setCursor(16, 10); gfx->print("fen.dog/kura");
...
void scrHome(uint32_t t) {
  ...
  holoText("KURA", 76, 140 + (int)bob, 14, t);
  gfx->setTextSize(3); gfx->setTextColor(C_TQ);
  gfx->setCursor(74, 280 + (int)(bob / 2)); gfx->print("mischievous kobold");
  gfx->setTextSize(2); gfx->setTextColor(C_DIM);
  gfx->setCursor(120, 330); gfx->print("KOBOLD - ARTIST - MISCHIEF");
  // boop chip
...
void scrFursona(uint32_t t) {
  particles(t);
  header("THE KOBOLD HIMSELF", t);
  float bob = sinf(t / 600.0f) * 5;
  gfx->draw16bitRGBBitmap(130, 105 + (int)bob, (uint16_t*)kura_art, KURA_ART_W, KURA_ART_H);
  // halo ring
  int r = 150 + (int)(sinf(t / 350.0f) * 6);
  gfx->drawCircle(240, 215 + (int)bob, r, holoAt(0, t));
  holoText("hi, i'm Kura!", 168, 370, 3, t);
  gfx->setTextSize(2); gfx->setTextColor(C_DIM);
...
void scrFilm(uint32_t t) {
  ...
  gfx->draw16bitRGBBitmap(38,  90 + (int)b1, (uint16_t*)kpol1, KPOL1_W, KPOL1_H);
  gfx->draw16bitRGBBitmap(230, 140 + (int)b2, (uint16_t*)kpol2, KPOL2_W, KPOL2_H);
  gfx->setTextSize(2); gfx->setTextColor(C_DIM);
...
void scrCinema(uint32_t t) {
  ...
  holoText("now playing: kobold.gif", 100, 390, 2, t);
}
...
void setup() {
  ...
  gifOpen = gif.open((uint8_t*)kura_gif, KURA_GIF_LEN, GIFDraw);
  ...
  wifiOk = wm.autoConnect("KURA-BADGE");
  if (wifiOk) poll();
}
